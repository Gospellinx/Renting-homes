...
const Index = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      {/* Compact dark green header using design system */}
      <div className="relative bg-primary">
        ...
        <header className="relative z-10 flex justify-between items-center px-6 py-3">
          ...
          {user ? (
            <Link to="/profile" className="flex items-center gap-2">
              ...
              <span className="text-primary-foreground font-medium text-sm hidden sm:inline">Profile</span>
            </Link>
          ) : (
            <div className="flex items-center gap-3">
              <Link to="/auth">
                <Button variant="ghost" className="text-primary-foreground hover:bg-primary-foreground/10 rounded-full px-5 text-sm">
                  Sign in
                </Button>
              </Link>
              <Link to="/auth">
                <Button className="bg-primary-foreground text-primary hover:bg-primary-foreground/90 rounded-full px-5 text-sm font-semibold">
                  Get Started
                </Button>
              </Link>
            </div>
        ...
        <main className="relative z-10 flex flex-col items-center px-4 pb-20 pt-6">
          <h1 className="text-primary-foreground text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-3">
            Looking to rent, buy, verify, or develop?
          </h1>

          {user && (
            <p className="text-primary-foreground/80 text-base sm:text-lg font-medium mb-5 text-center">
              Welcome back, {user.user_metadata?.full_name || user.email?.split("@")[0]} 👋
            </p>
          )}
          {!user && <p className="text-primary-foreground/60 text-sm mb-5">Search verified properties across Nigeria</p>}

          {/* Search Bar */}
          <form onSubmit={handleSearch} className="w-full max-w-2xl mb-8">
            <div className="relative flex items-center bg-card rounded-full shadow-md hover:shadow-lg transition-shadow">
              {isProcessing ? (
            ...
            {shortcuts.map((shortcut) => {
              ...
              return (
                <Link
                  key={shortcut.id}
                  to={shortcut.href}
                  className="flex flex-col items-center gap-2 p-3 rounded-lg transition-all hover:bg-primary-foreground/10 hover:scale-105"
                >
                  <div className="w-12 h-12 rounded-full border-2 border-primary-foreground/30 bg-primary-foreground/10 backdrop-blur-sm flex items-center justify-center">
                    <Icon className="h-5 w-5 text-primary-foreground" />
                  </div>
                  <span className="text-xs text-primary-foreground/90 font-medium max-w-[80px] text-center leading-tight">
                    {shortcut.label}
                  </span>
                </Link>
              );
            ...
            {adCards.map((card) => {
              ...
              return (
                <Link
                  key={card.href}
                  to={card.href}
                  className="group relative overflow-hidden rounded-lg border border-border bg-card hover:shadow-lg transition-all hover:scale-[1.02]"
                >
      ...
      <footer className="bg-card border-t border-border pb-6" />
    </div>
  );
};
